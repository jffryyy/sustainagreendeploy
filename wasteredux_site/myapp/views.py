...
def user_list(request):
    ...
    if search_term:
        ...
        wrusers = Users.objects.filter(query)
    else:
        wrusers = Users.objects.all()
...
def get_user_activity_data(request):
    ...
    for college in colleges:
        user_emails = Users.objects.filter(college_department=college).values_list('email_address', flat=True)

        scanned_counts = ScannedImage.objects.filter(email_address__in=user_emails)
        if year:
        ...
        unrecognized_counts = UnrecognizedImages.objects.filter(email_address__in=user_emails)
        if year:
